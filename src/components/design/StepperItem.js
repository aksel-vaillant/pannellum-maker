...
export default function StepperItem(props){
    ...
    let separatedText = props.name.split(" ").map(function(value) {
        return (
            <StepperValue key={value}>{value}<StepperSeparator/></StepperValue>  
        )
    })
