...
export default function PanGallery(props){
  ...
  const [userAction, setUserAction] = useState("");
    ...
    const getData = (data) =>{
      console.log(data);
      console.log(photos.filter((photos) =>{return photos.favorite}));
      //setUserAction(data);
    }
        ...
        <>
            ...
            <Row>
              ...
              <div className="flex w-72">
                <input className="inline-flex w-72 items-center content-center rounded pr-2.5 h-35 text-base leading-none text-black shadow-lg" type="text" name="searchBar" id="serachBar" placeholder="Recherchez..." onChange={handleSearchTerm}></input>  
              </div>
              ...
              <div className="searchResults">
                {photos
                  ...
                  .map((photos) => {
                    photoUpdate.push(photos);
                  })}
              </div>        
            </Row>
            {photoUpdate.length > 0 ? (<Gallery photos={photoUpdate} direction={"column"} margin={10}></Gallery>) : (<p>There's no files with this name</p>)}           
        </>
    )
}